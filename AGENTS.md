...
- Commands: `docker compose -f docker-compose.base44.yml exec -T wasm cargo test --workspace`, plus `... exec -T web npm run typecheck` and `... exec -T web npm run test:unit`. Test counts: Rust 237, vitest 206, Playwright 36. CI's `e2e` job runs Playwright (~7 min on GitHub's runner, ~12 on a 4-core container) on the engines the `web` job built, passed as an artifact. It runs against `vite preview` of `web/dist`, so rebuild before a local run. Retries are 0 on purpose, so fix a failing spec rather than retrying it. `npm run typecheck` also typechecks the specs (`tests/tsconfig.json`), which is the only thing that catches a spec importing a moved module. Update the README's Testing section when these counts change.
